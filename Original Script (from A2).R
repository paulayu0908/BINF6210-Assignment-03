...
Cigaritis.BOLD.clean <- tibble (processid = Cigaritis.BOLD$processid,     # use tibble to contruct this data frame with simple column name
                               species_name = Cigaritis.BOLD$species_name,   #a simple column name can reduce the error in downstream analysis
...
Cigaritis_Tree_0 <- (ape::nj(Cigaritis.OTU.DM))    #only use distance based method to build phylogeny tree is potential inaccurate
...
head(Cigaritis_Tree$tip.label)  #tibble is a subpackage in tidyverse which is aiming to create data frame in a simple way 
Tree_Table <- tibble(           # build the data.frame for country information in tidyverse style
...
#ggtree is a powerful package based on ggplot. It provides freedom and simpilicity in tree visulization. That chunk of codes can be replaced by 5 lines of codes.   
